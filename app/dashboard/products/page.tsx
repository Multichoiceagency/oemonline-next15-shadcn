...
export default function AdminProductsPage() {
  ...
  async function fetchTypes(msid: string) {
    ...
    try {
      ...
      setErr(e.message); setTypes([]); setYears([])
    } finally {
  ...
  async function onPickManu(id: string) {
    ...
    setModelSeriesId(""); setTypeId(""); setRows([])
  ...
  const filteredTypes = React.useMemo(() => {
    ...
    return types.filter((t) => {
      const from = Number(t?.raw?.yearOfConstrFrom)
      const to = Number(t?.raw?.yearOfConstrTo)
      if (Number.isFinite(from) && Number.isFinite(to)) return y >= from && y <= to
      ...
      <Card className="rounded-2xl">
        ...
        <CardContent className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={typeId} onValueChange={setTypeId} disabled={filteredTypes.length === 0 || loadingTypes}>
              <SelectTrigger className="h-9"><SelectValue placeholder={loadingTypes ? "Laden…" : (filteredTypes.length ? "Alle types" : "Kies model/bouwjaar")} /></SelectTrigger>
              <SelectContent className="max-h-80">
          ...
          <div className="flex items-center gap-2">
            ...
            <Input placeholder="bv. 30 (BOSCH) of 30,41" value={brandNo} onChange={(e) => setBrandNo(e.target.value)} />
          </div>
          <div className="flex items-center gap-2">
            <span className="w-28 text-sm text-muted-foreground">Zoek</span>
            <Input placeholder="naam / nummer" value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => e.key === "Enter" && run(1)} />
          </div>
